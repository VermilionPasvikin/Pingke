...
- 数据库：MySQL（推荐，生产环境）或SQLite（开发环境）
...
2. 服务默认运行在`http://0.0.0.0:5000`，API接口地址为`http://localhost:5000/api`
...
3. 运行Docker容器：
   ```bash
   docker run -d -p 5000:5000 --name pingke-backend pingke-backend
   ```
...
1. 打开`frontend/app.js`文件，修改API基础URL配置：
   ...
   globalData: {
     userInfo: null,
     apiBaseUrl: 'http://localhost:5000/api'  // 开发环境API地址（实际使用的地址）
...
3. 访问`http://localhost:5000/dashboard`查看性能监控数据
