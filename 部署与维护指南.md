...
- **tags**: 标签 (逗号分隔的字符串)
- **comment**: 评价内容
- **is_anonymous**: 是否匿名评价 (BOOLEAN，默认false)
- **user_name**: 用户昵称或"匿名用户"
- **created_at**: 创建时间 (TIMESTAMP)
- **updated_at**: 更新时间 (TIMESTAMP)
...
- **id**: 点赞ID (主键，INTEGER)
- **user_id**: 用户ID (外键关联users表，必填)
- **target_type**: 目标类型 ('comment' 或 'evaluation'，必填)
- **target_id**: 目标ID (被点赞对象的ID，必填)
- **created_at**: 创建时间 (TIMESTAMP)
...
- **course_id**: 课程ID (外键关联courses表，必填)
- **user_id**: 用户ID (外键关联users表，必填)
- **user_name**: 用户昵称 (兼容旧数据)
- **content**: 评论内容 (必填)
- **parent_id**: 父评论ID (自引用，用于回复功能，可选)
- **created_at**: 创建时间 (TIMESTAMP)
- **updated_at**: 更新时间 (TIMESTAMP)
...
│   │   │   ├── rankings_frontend.py # 前端排行榜API
│   │   │   ├── discussions.py # 讨论区相关API
│   │   │   └── users.py      # 用户相关API
│   │   ├── models/           # 数据模型
...
│   ├── data/                 # 数据相关脚本
│   │   └── test_data.py      # 测试数据
│   ├── fix_evaluation_user_names.py # 修复评价用户名称脚本
│   ├── migrate_add_evaluation_fields.py # 迁移评价字段脚本
│   └── venv/                 # 虚拟环境（可选）
├── frontend/                 # 前端微信小程序
│   ├── app.js                # 小程序入口
│   ├── app.json              # 全局配置
